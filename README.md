...
#### Installation
Install the plugin from `Dify Marketplace`, `Github`, `Local`, and put the API key in the configuration from.

![img1](_assets/README_assets/installation.png)

#### Add the plugin to Chatflow
You can add this plugin to either the Input or Output flow to detect suspicious content in the input or output. You must also use a `Conditional Node` to make the determination: if the detection result contains `true`, terminate the flow; if the detection result contains `false`, continue the flow.
...
- **json**: Return more detailed detection info so you can figure out which policy blocked it.
